...
(function() {
    ...
    //// Optimizations
    /// jMod takes special care to allow for optimal module loading
    /// by custom handlers.  The idea for the optimization is to load
    /// multiple modules and all of their dependencies at once.
    /// jMod tracks where this is possible and signifies this to the
    /// custom handler with the third argument as a boolean.

    var lump_requests = true;

    function disable_lump_requests() {
        lump_requests = false;
    }

    function should_lump_requests() {
        return lump_requests;
    }
    ...
    function request_modules() {
        ...
        _request_modules(rest);
        disable_lump_requests();
    }

    // Request a list of modules synchronously
    function include_modules() {
        modules = arguments_to_array(arguments);
        var rest = namespace_dispatch(modules, true, should_lump_requests());
        _request_modules(rest);
        disable_lump_requests();
    }

    function _request_modules(names) {
    ...
    function namespace_dispatch(names, block, optimize) {
        var unprocessed = [];
             ...
             function(namespace, names) {
                 if((handler = NAMESPACES[namespace])) {
                     if(typeof(handler) == "string") {
                         default_handler(handler, names, block, optimize);
                     }
                     else {
                         handler(names, block, optimize);
                     }
                 }
                 else {
    ...
    // from that url.
    function default_handler(base, names, block, optimize) {
        each(names, function(i, name) {
    ...
    function request(name, url, blocking) {
        if(!is_requested(name)) {
            request_forcefully(url || resolve_url(name),
                               blocking);
            REQUESTED[name] = blocking ? 'sync' : 'async';
        }
    }

    function request_forcefully(url, blocking) {
        if(blocking) {
    ...
    function register(module) {
        ...
        if(is_registered(name)) {
            warning("Module '" + name + "' is already registered.");
            return;
        }

        // This is required if any of the special namespace handlers
        // loading modules differently. We don't care that they
        // weren't requested through our request mechanism, as long as
        // they've showed up (which is why we're in this function).
        // Use case: consolidating all requests to the central jMod
        // libraries.
        if(!is_requested(name))
    ...
    function load(module) {
        ...
        try {
            // Even though module and args should be accessible from
            // the lexical scope, they must be passed in explicitly to
            // the module constructor to work around a "cannot access
            // optimized closure" bug in FF/firebug.  See also:
            //   https://bugzilla.mozilla.org/show_bug.cgi?id=505001
            //   https://bugzilla.mozilla.org/show_bug.cgi?id=496790
            obj = new (function (module, args) {
    ...
    window.jMod = {
        internal: {
            request: request,
            request_forcefully: request_forcefully,
